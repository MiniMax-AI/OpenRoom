...
function isAllowedLLMUrl(url: string): boolean {
  try {
    const { hostname } = new URL(url);
    return ALLOWED_LLM_HOSTS.some((h) => hostname === h || hostname.endsWith('.' + h));
  } catch {
    return false;
  }
}
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/llm-proxy', async (req, res) => {
        ...
        if (!isAllowedLLMUrl(targetUrl)) {
          res.writeHead(403, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'Target URL host not in allowlist' }));
          return;
        }
